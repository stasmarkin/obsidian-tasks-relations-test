...
# Search tests
```tasks
filter by function query.file.path == task.taskLocation.path && task.descriptionWithoutTags == "task1" && task.nextSibling == null && task.previousSibling == null && task.parent == null && task.children.length == 4 && task.children[0].descriptionWithoutTags == "task2" && task.children[1].descriptionWithoutTags == "task3" && task.children[2].descriptionWithoutTags == "task4" && task.children[3].descriptionWithoutTags == "task5"
```
> [!INFO]- Details
...
> ```
> ###    task1.children.length, expected 4
> ```tasks
filter by function query.file.path == task.taskLocation.path && task.descriptionWithoutTags == "task1" && task.children.length == 4
> ```
...
> ```
> ###    task1.children[3], expected task5
> ```tasks
filter by function query.file.path == task.taskLocation.path && task.descriptionWithoutTags == "task1" && task.children[3].descriptionWithoutTags == "task5"
> ```
```tasks
...
> ```
```tasks
filter by function query.file.path == task.taskLocation.path && task.descriptionWithoutTags == "task5" && task.nextSibling == null && task.previousSibling != null && task.previousSibling.descriptionWithoutTags == "task4" && task.parent != null && task.parent.descriptionWithoutTags == "task1" && task.children.length == 0
```
> [!INFO]- Details
